...
To update the database use this command from the Web project folder (replace `kriefTrackAiApi` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../kriefTrackAiApi.Infrastructure/kriefTrackAiApi.Infrastructure.csproj -s kriefTrackAiApi.Web.csproj
```
